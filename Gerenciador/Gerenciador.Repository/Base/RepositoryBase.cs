...
namespace Gerenciador.Repository
{
    public class RepositoryBase<T> : IDisposable where T : class
    {
        ...
        /// </summary>
        protected ConexaoDB Db = new ConexaoDB();
        /// <summary>
        ...
        public T Listar(int pId)
        {
            try
            {
                ...
                //(x => !((DateTime?)x.GetType().GetProperty("DataExclusao").GetValue(x)).HasValue)
        ...
        public void Excluir(int pId)
        {
            try
            {
                ...
                pObj.GetType().GetProperty("ATIVO").SetValue(pObj, 0);
                Db.SaveChanges();
            }
            catch (Exception ex)
            {
